...
func init() {
	...
	minersInfo = append(minersInfo, PeriodInfo{
		begin:      bonusBeginBlock,
		end:        bonusBeginBlock + 1,
		multiplier: DEFAULT_HEROES * DEFAULT_MULTIPLIER * 1000000000000 / BLOCK_PER_DAY, // mul 1e12
	})

	minersInfo = append(minersInfo, PeriodInfo{
		begin:      bonusBeginBlock + 2,
		end:        0,
		multiplier: 200 * DEFAULT_MULTIPLIER * 1000000000000 / BLOCK_PER_DAY, // mul 1e12
	})
}
...
func main() {
	...
	from = 101
	to = 650
	reward(from, to) // (497-101) * 0.333) + (650 - 498) * 0.666
	fmt.Println("wright:", v1*(497-101)+(650-498)*v2)

	from = 101
	to = 880
	reward(from, to)
	fmt.Println("wright:", v1*(497-101)+(697-498)*v2+(880-698)*v3)

	from = 101
	to = 1000
	reward(from, to)
	fmt.Println("wright:", v1*(497-101)+(697-498)*v2+(897-698)*v3+(1000-898)*v4)
	...
	to = 880
	reward(from, to)
	fmt.Println("wright:", (697-580)*v2+(880-698)*v3)
}
